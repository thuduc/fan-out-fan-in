...
class TaskDescriptor:
    """Describes a single task within a group for dispatch to vnas."""
    request_id: str
    group_index: int
    group_name: str
    task_id: str
...
class RequestOrchestrator:
    """
    Orchestrates sequential group processing for a single request.
    Enforces group ordering, generates task XMLs, dispatches to vnas,
    monitors completion, and assembles final response.
    """
    def __init__(
        self,
    ...
    def run(self, event: Dict[str, str]) -> Dict[str, str]:
        """
        Main entry point. Processes all groups sequentially, then assembles response.
