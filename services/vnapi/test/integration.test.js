...
const REQUEST_PYTHON = resolvePythonExecutable('vnvs');
const REQUEST_RUNNER = path.join(repoRoot, 'services/vnvs/app/local_runner.py');
...
test('integration: sync submission returns composed response', { concurrency: false }, async (t) => {
  const harness = await createHarness(REQUEST2_XML_PATH, t);
  const { submissionService, queryService, xml } = harness;
